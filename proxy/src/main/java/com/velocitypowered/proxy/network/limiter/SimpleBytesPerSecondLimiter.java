...
public final class SimpleBytesPerSecondLimiter implements PacketLimiter {
  ...
  /**
   * Counter that tracks the moving average of bytes per second,
   * or {@code null} if no byte limit is enforced.
   */
  @Nullable
  private final IntervalledCounter bytesCounter;

  /**
   * Counter that tracks the moving average of packets per second,
   * or {@code null} if no packet limit is enforced.
   */
  @Nullable
    private final IntervalledCounter packetsCounter;

  /**
   * Maximum allowed packets per second.
   */
   ...
   */
  public SimpleBytesPerSecondLimiter(final int packetsPerSecond, final int bytesPerSecond, final int windowSeconds) {
  ...
  @Override
  public boolean account(final int bytes) {
