...
public class QueueManagerImpl implements QueueManager {
    ...
    public QueueManagerImpl(VelocityServer proxyServer){
        ...
        new Thread(()->{
            proxyServer.getScheduler().buildTask(proxyServer, ()->{
                tick();
            }).repeat((long) velocityServer.getQueueConfiguration().sendDelay(), TimeUnit.SECONDS).schedule();
        }).start();

    }

    public boolean isServerRestarting(String serverName){
        if(!velocityServer.getRedisConfiguration().proxyId().equals(velocityServer.getRedisConfiguration().masterProxyId())) return false;
    ...
    public synchronized void addRestartingServer(String serverName){
        ...
        velocityServer.getScheduler().buildTask(velocityServer, ()->{
            serversRestarting.remove(serverName);
        }).delay(Duration.ofSeconds((long) velocityServer.getQueueConfiguration().offlineDelay())).schedule();
    }
    ...
    private void tick(){
        for(String server : queueMap.keySet().stream().toList()){
            ...
            for(int i = 0; i < entries.size(); i++){
                ...
                velocityServer.getRedisManager().send(new QueueSendMessageObj(entry.getUuid().toString(), "You are currently #" + (i + 1) + " in the queue - " +
                        status.get()));
            }
            ...
            velocityServer.getRedisManager().send(new QueueSendObj(queueEntry.getUuid().toString(), server));
        }
    }
    ...
    public void add(String serverName, UUID player, int priority){
        ...
        LinkedList<QueueEntry> queue = queueMap.getOrDefault(serverName, new LinkedList<>());
