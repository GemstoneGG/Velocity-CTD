...
/**
 * The {@code QueueSendMessageHandler} class is responsible for handling "QueueSendMessageObj" messages
 * received via the Redis Pub/Sub system.
 * <p>
 * This handler processes the JSON payload, retrieves the relevant {@link Player}, and sends a message
 * to that player using the content specified in the message.
 * </p>
 */
public class QueueSendMessageHandler implements PubSubHandler {
  ...
  public void handle(String jsonString) {
    ...
    if (!(jsonObject.get("type").getAsString().equals("QueueSendMessageObj"))) {
      return;
    }
